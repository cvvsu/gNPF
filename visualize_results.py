...
if __name__ == '__main__':
    ...
    savefp = os.path.join('figures', args.station)
    os.makedirs(savefp, exist_ok=True)
    ...
    net = GNPF()
    epoch = 20
    net.load_state_dict(torch.load(f'checkpoints/{args.station}/G_{epoch}.pth', map_location=device))
    net.eval()

    # dataframe with template time points and particle size slots
    df_temp = pd.read_csv('template.csv', parse_dates=[0], index_col=0)

    # visualize the first 8 results
    n = 8
    with torch.no_grad():
        torch.manual_seed(args.seed)
        x = torch.randn(n, 100, 1, 1)
        y = net(x).squeeze().cpu().numpy() * args.vmax
        dfs = [pd.DataFrame(y_, index=df_temp.index, columns=df_temp.columns) for y_ in y]
        draw_subplots(dfs, names=['']*n, nrows=2, ncols=4, savefp=savefp, vmaxs=[args.vmax]*n, savename=args.station)



    # number of random results
    # num_seed = 2000
    # savefp_ = os.path.join(savefp, 'gPNSD')
    # os.makedirs(savefp_, exist_ok=True)

    # # generate and draw PNSD with random seeds
    # with torch.no_grad():
    ...
    #         psd2im(df, show_figure=False, savefp=savefp_, vmax=args.vmax, index=f'{seed}', figsize=(6, 2.5), use_cbar=True)
    ...
    #             y = net(z).squeeze().cpu().numpy() * args.vmax
    #             df = pd.DataFrame(y, index=df_temp.index, columns=df_temp.columns)
    #             dfs.append(df)
    #     draw_subplots(dfs, names=['']*len(dfs), nrows=2, ncols=6, savefp=savefp, vmaxs=[args.vmax]*len(dfs), savename=f'{seed1}-{seed2}')
